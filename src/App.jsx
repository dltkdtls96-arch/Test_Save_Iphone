    ...
    return ( <
        ...
        style = {
            ...
            selectedTab === "home" && ( <
                  ...
                  (homePage === 0 ? 0 : -slideViewportH) + dragYHome
                            ...
                            transition: snapYHome ?
                                `transform ${V_SNAP_MS}ms ease-out` : "none",
                            willChange: "transform",
                        }
                    }
                    onTransitionEnd = {
                        vHome.onTransitionEnd
                    } > {
                        /* Panel 0: 캘린더 */
                    } <
                    div ref = {
...

                    {
                        ...
                        fmt(selectedDate) !== fmt(today) && ( <
                            button className = "px-2 py-1 rounded-xl bg-indigo-500 text-white text-xs"
                            onClick = {
                                () => {
                                    setSelectedDate(today);
                    ...
                    select className = "bg-gray-700 rounded-xl p-1 text-xs"
                    value = {
                        selectedDepot
                    }
                    onChange = {
                        (e) => setSelectedDepot(e.target.value)
                    } > {
                        DEPOTS.map((d) => ( <
                            option key = {
                                d
                        ...
                        [-1, 0, 1].map((offset) => {
                                ...
                                return ( <
                                        ...
                                        style = {
                                            {
                                                ...
                                                gridTemplateRows: compressLastRow ?
                                                    `repeat(5, minmax(0,1fr)) minmax(0, 0.66fr)` : "repeat(6, minmax(0,1fr))",
                                            }
                                        } > {
                                            monthDays.map((d, i) => {
                                  ...
                                  t.out
                                                        ...
                                                        <
                                                        div className = {
                                                            `break-words text-[clamp(12px,1vw,11px)] leading-tight ${diaColorClass} mb-[4px]`
                                                        } > {
                                                            diaLabel
                                                        } <
                                                        /div> <
            ...
            title = "날짜 선택" /
                ...
                /span> {
            fmt(selectedDate) !== fmt(today) && ( <
                button className = "px-2 py-1 rounded-xl bg-indigo-600 text-white text-xs hover:bg-indigo-500 active:scale-[.98] transition"
                onClick = {
        ...
        data - no - gesture >
        <
        button className = "rounded-full px-3 py-1 text-sm bg-cyan-600 text-white"
        onClick = {
            () =>
...

{
    ...
    selectedTab === "roster" && ( <
            div className = "bg-gray-800 rounded-2xl p-3 shadow mt-4"
            style = {
                {
                    minHeight: slideViewportH
                }
    ...
    data - no - gesture >
        ...
        <
        span className = "text-xs text-gray-300" > 소속 < /span> <
    select className = "bg-gray-700 rounded-xl px-2 py-1 text-sm"
    value = {
        selectedDepot
    }
    onChange = {
...

{
    ...
    selectedTab === "route" && ( <
            ...
            <
            select className = "bg-gray-700 rounded-xl px-2 py-1 text-xs"
            value = {
                selectedDepot
            }
            onChange = {
            ...
            {
                routeTargetName && ( <
                    button className = "px-2 py-1 rounded-xl bg-orange-700 hover:bg-gray-600 text-xs"
                    onClick = {
                        () => setRouteTargetName("")
                    }
                    title = "내 이름으로 보기" >
            ...
            select className = "bg-gray-700 rounded-xl p-1 text-sm"
            value = {
                routeTargetName || myName
            }
            onChange = {
            ...
            onTouchStart = {
                swipeRouteP0.onStart
            }
            onTouchMove = {
                swipeRouteP0.onMove
            }
            onTouchEnd = {
                swipeRouteP0.onEnd(goPrevDay, goNextDay)
            }
            style = {
                swipeRouteP0.style
            } > {
...

                                {
                                    (() => {
                                            ...
                                            return ( <
                                                    ...
                                                    div className = "relative w-full aspect-[1/1.414] select-none"
                                                    onDoubleClick = {
                                                        toggleFlip
                                                    } // 데스크탑 더블클릭
                                                    onTouchEnd = {
...
function CompareWeeklyBoard({
    ...
    return ( <
            ...
            onChange = {
                (e) => setPickerDepot(e.target.value)
            }
            title = "사람 추가용 소속 선택" > {
                DEPOTS.map((d) => ( <
                    option key = {
            ...
            <
            button className = "ml-2 px-2 py-1 rounded-xl bg-indigo-600 text-white text-xs"
            onClick = {
                () => setPickerOpen((v) => !v)
            } > {
            ...
            /button> <
            button className = "px-2 py-1 rounded-xl bg-gray-700 text-xs"
            onClick = {
                resetToMine
            }
            title = "내 이름만 남기기" >
            ...
            (fmt(selectedDate) !== todayISO ||
                !displayedWeekDays.some((d) => fmt(d) === todayISO)) && ( <
                button className = "px-2 py-1 rounded-xl bg-indigo-600 text-white text-xs"
                onClick = {
                    jumpToToday
                }
                title = "오늘로" >
    ...
    div > {
            ...
            pickerOpen && ( <
                div className = "mt-2 p-2 rounded-xl bg-gray-900 shadow-lg border border-gray-700"
                data - no - gesture style = {
                    {
                        position: "relative",
                        zIndex: 3,
                ...
                <
                input className = "flex-1 bg-gray-700 rounded-xl px-2 py-1 text-sm"
                placeholder = "이름 검색…"
                value = {
                    filterText
                }
    ...
    style = {
        ...
        todayColIndex >= 0 && ( <
            div className = "absolute pointer-events-none border-2 border-red-400 rounded-md"
            style = {
                {
                    top: 0,
    ...
    style = {
        ...
        <
        div className = "grid rounded-t-xl overflow-hidden"
    style = {
            {
            ...
            weeks.map((weekDays, wi) => ( <
                ...
                >
                {
                    people.map(({
                        ...
                        <
                        div className = "text-white font-semibold truncate text-[12px] min-w-0"
                        title = {
                            `${depot} • ${name}`
                        } > {
                            name
                        ...
                        /div> <
                        button className = "w-4 h-4 rounded-full bg-gray-700 hover:bg-gray-600 text-[10px] flex items-center justify-center flex-shrink-0 ml-0.5"
                        onClick = {
                            () => removePerson(name, depot)
                        }
                        title = {
                            `${name} 해제`
                        } > −
...

                        {
                            ...
                            weekDays.map((d) => {
                                ...
                                return ( <
                                    ...
                                    className = {
                                        `px-1 py-1 text-[12px] leading-tight border-l border-gray-700 ${bgColor} ${
                            outside ? "opacity-50" : ""
                          }`
                                    }
                                    title = {
                                        `${depot} • ${name} • ${fmtWithWeekday(
                            d
                          )} • DIA ${dia} / ${t.in}~${t.out}`
                                    } >
                                    <
    ...
    div >
        ...
        <
        div className = "mt-1 text-[10px] text-gray-400 text-center" > ←오른쪽: 다음달 / 왼쪽: 전달· 위 / 아래: 주 변경 <
        /div> < /
    div >
);
}

function DiaOrderSections({
    diaViewData,
    nightDiaThreshold
}) {
    return ( <
